...
class Comments extends Component {
  ...
  getComments = async () => {
    let response = await axios.get(
      `http://localhost:5000/api/comments/get/${this.props.videoId}`
    );
              ...
              <div className="row">
                <div className="col-6 col-sm-4">
                  <Button
                    ...
                    onClick={() => this.likeComment(parent.id)}
                  >
                    Like ({parent.likes})
                  </Button>
                </div>
                <div className="col-6 col-sm-4">
                  <Button
                    ...
                    onClick={() => this.dislikeComment(parent.id)}
                  >
  ...
  dislikeComment = async (commentId) => {
    try {
      await axios.get(
        `http://localhost:5000/api/comments/dislike/${commentId}`
      );
      this.getInitialState();
    } catch (err) {
      alert(err);
    ...
    return (
      <div className="mt-4">
        ...
        {this.state.parents ? (
          <React.Fragment>
            <div
              className="jumbotron overflow-auto text-md-sm"
